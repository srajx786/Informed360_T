...
/*
 * Informed360 server — includes /api/topics for aggregated clusters.
 * Front-end uses:
 *  - /api/news   (latest articles with VADER)
 *  - /api/topics (aggregated clusters: avg sentiment + #sources)
 *  - /api/pinned (optional pin list from rss-feeds.json)
 *  - /api/ticker (Sensex/Nifty/NYSE)
 */
...
const extractImage = (item) => {
  ...
  const d = domainFromUrl(item.link || ""); return d ? `https://logo.clearbit.com/${d}` : "";
};

// VADER → {posP,negP,neuP,label,reasons[]}
...
// Topic key by bigrams to cluster similar headlines
...
// Aggregate into clusters: {title,count,sources, sentiment:{pos,neg,neu}}
...
/* ---------------- API ---------------- */
app.get("/api/news",   (req,res)=>{ const limit = Number(req.query.limit || 200); res.json({ fetchedAt:CACHE.fetchedAt, articles: CACHE.articles.slice(0, limit) }); });
app.get("/api/topics", (req,res)=>{ res.json({ fetchedAt: CACHE.fetchedAt, topics: computeClusters(CACHE.articles) }); });
